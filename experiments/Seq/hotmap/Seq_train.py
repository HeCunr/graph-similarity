    ...
    def train_epoch(self, dataloader, epoch):
        ...
        for batch_idx, (entity_type, entity_params) in enumerate(pbar):
            try:
                ...
                # 给 contrastive_loss 传递是否画图的标志
                if batch_idx == 0:
                    # 设置当下 epoch 号（用于命名热力图文件）
                    self.contrastive_loss.current_epoch = epoch + 1
                    self.contrastive_loss.save_heatmap = True
                else:
                    self.contrastive_loss.save_heatmap = False
                # 两份增强
