...
from utils import simgnn_utils, CGMN_utils
...
class CFGDataset(object):
    def __init__(self, data_dir, batch_size):
        ...
        graphs_train, classes_train, graphs_dev, classes_dev, graphs_test, classes_test = self.partition_graph_dataset(
            self.gs, self.classes, [0.5, 0.3, 0.2], perm)
        ...
        self.valid_epoch, valid_ids = CGMN_utils.generate_epoch_pair(graphs_dev, classes_dev, batch_size, output_id=True)
        ...
        self.test_epoch, test_ids = CGMN_utils.generate_epoch_pair(graphs_test, classes_test, batch_size, output_id=True)
    ...
    def read_graph(self):
        ...
        for file in os.listdir(self.data_dir):
            ...
            with open(f_name) as inf:
                for line in inf:
                    g_info = json.loads(line.strip())
                    label = self.func_name_dict[g_info['fname']]#字典中对应的值‘’aaa":1,label=1
                    classes[label].append(len(graphs))
                    cur_graph = CGMN_utils.graph(node_num=g_info['n_num'], label=label, name=g_info['src'])
