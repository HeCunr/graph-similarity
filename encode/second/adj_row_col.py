...
def get_feature_vectors(dxf_file_path):
    ...
    overlapping_counts = {
        etype: {other_type: 0 for other_type in ENTITY_TYPES}
    ...
    for entity in entities:
        ...
        for match_id in possible_matches:
            if match_id == entity_id:
                continue  # 跳过自身
            if match_id >= len(entities):
                print(f"Warning: match_id {match_id} is out of range for entities with length {len(entities)}")
    ...
    feature_vectors = {}
    for etype in ENTITY_TYPES:
        # 计算不与任何其他实体类型重叠的实体数量（自身类）
        non_overlapping_count = total_counts[etype] - len(overlapping_entities[etype])

        # 构建特征向量，第一列为自身类数量，后续列为与所有实体类型（包括自身类型，重叠数量设为0）的重叠数量
        vector = [non_overlapping_count]  # SELF 数量
...
def normalize_feature_vectors(feature_vectors):
    ...
    对特征矩阵进行归一化：
    - 第一列（SELF）：对所有行进行列归一化
    - 后续列（重叠数量）：对每一行进行行归一化（不包含第一列）
    """
    # 将特征向量转换为矩阵形式，按照 ENTITY_TYPES 的顺序
    feature_matrix = []
    for etype in ENTITY_TYPES:
        feature_matrix.append(feature_vectors[etype])

    # 提取第一列（SELF）进行列归一化
    first_column = [row[0] for row in feature_matrix]
    col_sum = sum(first_column)
    if col_sum != 0:
        normalized_first_column = [value / col_sum for value in first_column]
    else:
        normalized_first_column = first_column  # 如果总和为0，保持原始值

    # 对后续列（重叠数量）进行行归一化
    normalized_rows = []
    for i, row in enumerate(feature_matrix):
        self_value = normalized_first_column[i]  # 已归一化的自身类值
        other_values = row[1:]  # 其他实体类型的值（包括自身类型，重叠数量为0）
        row_sum = sum(other_values)
        if row_sum != 0:
            normalized_other_values = [value / row_sum for value in other_values]
        else:
            normalized_other_values = other_values  # 如果总和为0，保持原始值
        normalized_row = [self_value] + normalized_other_values
...
def get_normalized_feature_matrix(dxf_file_path):
    ...
    normalized_matrix = normalize_feature_vectors(feature_vectors)
    print('特征矩阵：')
    print(normalized_matrix)
    return normalized_matrix

def print_feature_matrix(normalized_matrix):
    """
    打印归一化后的特征矩阵
    """
    # 构建表头，第一列为 'SELF'，后续列为 ENTITY_TYPES
    header = ['SELF'] + ENTITY_TYPES
    print('\t'.join(['实体类型'] + header))
    for i, etype in enumerate(ENTITY_TYPES):
        row = [etype] + [str(round(value, 4)) for value in normalized_matrix[i]]
        print('\t'.join(row))

if __name__ == '__main__':
    dxf_file_path = r'C:\Users\15653\dwg-cx\dataset\modified\DFN6LCG(NiPdAu)（321）-517  Rev1_1.dxf'  # 请替换为您的DXF文件路径
    normalized_matrix = get_normalized_feature_matrix(dxf_file_path)
    print("归一化的特征矩阵：")
    print_feature_matrix(normalized_matrix)
