...
from SimSeq import SimSeq  # 直接使用SimSeq.py中的实现
...
class FMRDEvaluator:
    ...
    def evaluate(self, top_n=10):
        ...
        for target_file in tqdm(self.ground_truth, desc="Evaluating"):
            ...
            # 获取推荐列表（排除自身）
            recommended = [item[0] for item in self.sim_matrix[target_file][:top_n]]
...
def main():
    ...
    parser.add_argument("--ground_truth", default=r"/home/vllm/encode/data/Seq/Seq_truth.json",help="Path to ground truth JSON")
    parser.add_argument("--folder_path", default=r"/home/vllm/encode/data/Seq/TEST_4096", help="Folder containing .h5 files")
    parser.add_argument("--model_path", default=r"/home/vllm/encode/pretrained/Seq/Seq.pth", help="Trained model checkpoint path")
    parser.add_argument("--top_n", type=int, default=10, help="Number of top similar files to retrieve")
    parser.add_argument("--gpu_id", type=int, default=0, help="GPU ID to use")
    args = parser.parse_args()

    # 初始化SimSeq（直接使用SimSeq.py的实现）
    ...
    for group in raw_truth:
        for file in group:
            ground_truth[file] = [f for f in group if f != file]  # 排除自身
